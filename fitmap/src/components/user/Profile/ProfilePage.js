...
function ProfilePage() {
  ...
  const getDefaultMetric = (challenge) => {
    ...
    if (
      ...
      lowerText.includes('ק"מ') ||
      lowerText.includes("מרחק")
    ) {
      return "km";
    } else if (
      lowerText.includes("זמן") ||
      lowerText.includes("דקות") ||
      lowerText.includes("שעות")
    ) {
      return "minutes";
  ...
  // פונקציה לטיפול בשינוי העדפות אימון
  const handleWorkoutTypeChange = (typeId, isSelected) => {
    // המרה למחרוזת
    const workoutId = String(typeId);

    setProfileData((prevData) => {
      // וידוא שקיים מערך
      const currentWorkouts = Array.isArray(prevData.preferredWorkouts)
        ? [...prevData.preferredWorkouts]
        : [];

      if (isSelected) {
        // הוספת סוג האימון אם הוא לא קיים כבר
        if (!currentWorkouts.includes(workoutId)) {
          return {
            ...prevData,
            preferredWorkouts: [...currentWorkouts, workoutId],
          };
        }
      } else {
        // הסרת סוג האימון
      ...
      // החזרת המצב המקורי אם לא נדרש שינוי
      return prevData;
    });

    console.log(`סוג אימון ${typeId} ${isSelected ? "נבחר" : "בוטל"}`);
  };

  // פונקציה משופרת לשליחת פרטי הפרופיל לשרת
  ...
  const handleSubmit = async (updatedData) => {
    ...
    try {
      ...
      console.log("Form data for submission:", updatedData);
      ...
      // המרת העדפות אימון לפורמט PostgreSQL
      let workoutsArray = [];
      if (Array.isArray(updatedData.preferred_workouts)) {
        workoutsArray = updatedData.preferred_workouts.map((id) => String(id));
      } else if (Array.isArray(updatedData.preferredWorkouts)) {
        workoutsArray = updatedData.preferredWorkouts.map((id) => String(id));
      }

      // הכנת האובייקט לשליחה
      const serverData = {
        name: updatedData.name,
        email: updatedData.email, // חובה
        phone: updatedData.phone,
        fitness_level: updatedData.fitness_level,
        preferred_workouts: `{${workoutsArray.join(",")}}`, // פורמט PostgreSQL
      ...
      console.log("Data being sent to Supabase:", serverData);
      ...
      // נסה לקבל את הנתונים המעודכנים מהשרת
      ...
      if (fetchError) {
        ...
        // המשך בכל מקרה גם אם לא הצלחנו לקבל את הנתונים המעודכנים
      }

      // עדכון ה-state המקומי
      const parsedWorkouts = workoutsArray; // כבר המרנו למערך קודם

      setProfileData((prev) => ({
        ...prev,
        ...(updatedProfile || {}),
        // וודא שה-state שלנו מכיל גם את הגרסה ב-camelCase
        name: updatedData.name,
        preferredWorkouts: parsedWorkouts,
        fitnessLevel: updatedData.fitness_level,
