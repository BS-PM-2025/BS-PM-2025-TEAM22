...
function ChallengesTab({ challenges, loading, navigate, formatDateHebrew }) {
  ...
  const getChallengeIcon = (iconName) => {
    switch (iconName) {
      case "trophy":
        return <FaTrophy className={styles.challengeIcon} />;
      case "running":
        return <FaRunning className={styles.challengeIcon} />;
      case "calendar":
        return <FaCalendar className={styles.challengeIcon} />;
      case "dumbbell":
        return <FaDumbbell className={styles.challengeIcon} />;
      default:
        return <FaStar className={styles.challengeIcon} />;
    }
  };
                ...
                <div className={styles.challengeDates}>
                  {challenge.start_date && (
                    <span className={styles.startDate}>
                      <FaCalendarAlt className={styles.dateIcon} /> התחלה:{" "}
                      {formatDateHebrew(challenge.start_date)}
                    </span>
                  )}
                  {challenge.end_date && (
                    <span className={styles.endDate}>
                      <FaCalendarAlt className={styles.dateIcon} /> סיום:{" "}
                      {formatDateHebrew(challenge.end_date)}
                    </span>
                  )}
                </div>
              )}
